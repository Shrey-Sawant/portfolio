...
// ─── CARD COMPONENT ─────────────────────────────────────────────
const Card = forwardRef(({ title, img, bg, backBg, services }, ref) => {
  ...
  return (
    <div
      ...
      style={{ width: 340, height: 520, transformStyle: "preserve-3d" }}
    >
      {/* FRONT */}
      <div
        ref={floatRef}
      ...
      >
        ...
        <div className="flex justify-between items-end p-5">
          ...
          <p className="text-[11px] font-bold uppercase tracking-widest opacity-80" style={{ transform: "rotate(180deg)" }}>
            {title}
          </p>
        </div>
      </div>
      ...
      >
        ...
        <div className="flex justify-between items-center px-5 py-4 border-t border-black/10">
          ...
          <p className="text-[11px] font-bold uppercase tracking-widest opacity-50" style={{ transform: "rotate(180deg)" }}>
            {title}
          </p>
        </div>
      </div>
...
const HeroCards = () => {
  ...
  // Handle Locomotive Scroll custom progress event
  ...
  useEffect(() => {
    window.addEventListener("progressHero", handleProgress);
    ...
    createTimeline();
    return () => {
      window.removeEventListener("progressHero", handleProgress);
    };
  }, []);
  ...
  return (
    ...
    >
      <div className="relative flex items-center justify-center w-full">
        {Cards.map((card, i) => (
          <Card key={i} {...card} ref={(el) => (cardRefs.current[i] = el)} />
        ))}
      </div>
    </section>
  );
